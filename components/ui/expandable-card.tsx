  ...
  return (
    <>
        ...
        {active && (
          <div className={cn("fixed inset-0 grid place-items-center z-[100] sm:mt-16 before:pointer-events-none")}>
            <motion.div
              ...
              ref={cardRef}
              className={cn(
                "w-full max-w-[850px] h-full flex flex-col overflow-auto [scrollbar-width:none] [-ms-overflow-style:none] [-webkit-overflow-scrolling:touch] sm:rounded-t-3xl bg-zinc-50 shadow-sm dark:shadow-none dark:bg-muted relative",
                classNameExpanded,
              )}
              {...props}
            >
              <motion.div layoutId={`image-${title}-${id}`}>
                <div className="relative before:absolute before:inset-x-0 before:bottom-[-1px] before:h-[70px] before:z-50 before:bg-gradient-to-t dark:before:from-zinc-950 before:from-zinc-50">
                  <img src={src || "/placeholder.svg"} alt={title} className="w-full h-80 object-cover object-center" />
                </div>
              </motion.div>
                ...
                <div className="flex justify-between items-start p-8 h-auto">
                  <div>
                    ...
                    <motion.p
                      ...
                      className="text-zinc-500 dark:text-zinc-400 text-lg pt-2"
                    >
                      {description}
                    </motion.p>
                  </div>
      ...
      <motion.div
        ...
        className={cn(
          "p-3 flex flex-col justify-between items-center bg-card shadow-sm dark:shadow-none rounded-2xl cursor-pointer border border-gray-200/70 dark:border-zinc-900",
          className,
        )}
      >
