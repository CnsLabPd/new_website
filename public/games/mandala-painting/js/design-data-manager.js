...
class DesignDataManager {
    ...
    async saveDesign(designData) {
        try {
            ...
            // Sync to Supabase if API is available
            if (window.MandalaPaintingAPI && window.NeurogatiAuth && window.NeurogatiAuth.isAuthenticated()) {
                try {
                    console.log('☁️ DESIGN: Syncing to Supabase...');
                    const api = new window.MandalaPaintingAPI();
                    await api.saveDesign(dataToSave);
                    console.log('✅ DESIGN: Synced to Supabase successfully');
                } catch (error) {
                    console.error('❌ DESIGN: Failed to sync to Supabase:', error);
                    // Don't throw - local save succeeded, cloud sync is optional
                }
            } else {
                console.log('ℹ️ DESIGN: Skipping Supabase sync (API not available or user not authenticated)');
            }
