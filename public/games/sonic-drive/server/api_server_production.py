...
# For Vercel deployment, static files are served directly by Vercel, not Flask
# Only API routes should be handled by this Flask app
app = Flask(__name__)
...
# Initialize database with lazy connection (don't connect until first use)
db = None
...
def register():
    ...
    try:
        ...
        user_id = get_db().create_user(username, password_hash)
        ...
        get_db().create_session(user_id, token, expires_at)
...
def login():
    ...
    try:
        ...
        token = generate_token()
        expires_at = datetime.now() + SESSION_DURATION
        get_db().create_session(user['id'], token, expires_at)
...
def get_progress():
    ...
    try:
        ...
        progress = get_db().get_user_progress(user['id'])
...
def get_level_progress(level_number):
    ...
    try:
        ...
        progress = get_db().get_level_progress(user['id'], level_number)
...
def save_progress():
    ...
    try:
        ...
        # Save progress
        get_db().save_progress(user['id'], level_number, module_number, score, completed)

        # Get updated progress
        updated_progress = get_db().get_level_progress(user['id'], level_number)
...
def get_leaderboard(level_number):
    ...
    try:
        ...
        leaderboard = get_db().get_leaderboard(level_number, limit)
...
def get_user_level_history(username, level_number):
    ...
    try:
        ...
        # Get score history
        score_history = get_db().get_user_score_history(user['id'], level_number, limit)
...
# STATIC FILE SERVING
# ============================================================================
# NOTE: For Vercel deployment, static files are served directly by Vercel CDN
# These routes are only used for local development with standalone Flask server
...
def cleanup_expired_sessions():
    ...
    deleted = get_db().cleanup_expired_sessions()
    if deleted > 0:
...
if __name__ == '__main__':
    ...
    if hasattr(get_db(), 'db_path'):
        print(f"📊 Database: {get_db().db_path} (SQLite)")
    elif hasattr(get_db(), 'database_url'):
