...
class AudioManager {
    ...
    createBombExplosionSound() {
        return () => {
            ...
            // Softer, less irritating collision sound
            const duration = 0.25;  // Reduced from 0.5s to 0.25s
            const noise = this.context.createBufferSource();
            const gainNode = this.context.createGain();
            const filter = this.context.createBiquadFilter();

            // Create noise buffer (softer noise)
            const bufferSize = this.context.sampleRate * duration;
            const buffer = this.context.createBuffer(1, bufferSize, this.context.sampleRate);
            const data = buffer.getChannelData(0);
            for (let i = 0; i < bufferSize; i++) {
                data[i] = (Math.random() * 2 - 1) * 0.6;  // Reduce noise intensity
            }
            noise.buffer = buffer;

            // Higher frequency filter for less harsh sound
            filter.type = 'lowpass';
            filter.frequency.setValueAtTime(400, this.context.currentTime);  // Increased from 200Hz
            filter.Q.setValueAtTime(0.7, this.context.currentTime);  // Softer filter

            // Much quieter volume envelope
            const volume = this.settings.sfxVolume * 0.25;  // Reduced from 0.7 to 0.25
            gainNode.gain.setValueAtTime(volume, this.context.currentTime);
            gainNode.gain.exponentialRampToValueAtTime(0.001, this.context.currentTime + duration);
